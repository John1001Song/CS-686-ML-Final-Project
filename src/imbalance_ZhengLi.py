...
def Knn_classifier(X_train, train_target, X_test):
    knn = KNeighborsClassifier()
    knn.fit(X_train, train_target)
    hyp = knn.predict(X_test)
    return hyp
...
if __name__ == "__main__":
    paths = ['../arff/PONZI_100000.arff','../arff/PONZI_basic_100000.arff','../arff/PONZI_opcodes_100000.arff']
    for path in paths:
        print('\n' + path + '\n')
        ...
        hyp = decision_tree(x_train_over, y_train_over, X_test)
        print('Accuracy(Resampled DecisionTree) is %s' % accuracy_score(hyp, y_test))
        ...
        hyp = XGBoost_classifier(x_train_over, y_train_over, X_test)
        print('Accuracy(Resampled XGB Classifier) is %s' % accuracy_score(hyp, y_test))
        print('Confusion matrix:\n %s' % confusion_matrix(hyp, y_test))

        hyp = Knn_classifier(x_train_over, y_train_over, X_test)
        print('Accuracy(Resampled KNN Classifier) is %s' % accuracy_score(hyp, y_test))
        print('Confusion matrix:\n %s' % confusion_matrix(hyp, y_test))
        ...
        #
        # xgb = XGBClassifier()
        # scores = cross_val_score(xgb, df_X, df_target, cv=10)
        # print('XGBoost accuracy is %s' % np.average(scores))
